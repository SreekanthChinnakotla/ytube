...
const Navbar = () => {
    ...
    return (
        <div className="flex  top-0 justify-center items-center w-[100%] z-10 bg-white">
            <div className="flex w-[96%] py-2  justify-between items-center">

                <div className="flex items-center ">
                    <GiHamburgerMenu onClick={toggleHandler} size="24px" className="cursor-pointer" />
                    <img className="px-4" width={"125px"} src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/YouTube_Logo_2017.svg/768px-YouTube_Logo_2017.svg.png" alt="yt_logo" />
                </div>
                <div className="flex w-[40%] items-center">
                    <div className="flex w-[100%] ">
                        <input value={input} onFocus={openSuggestion} onKeyDown={(e)=>{if(e.key==='Enter') searchVideo()}} onChange={(e) => setInput(e.target.value)} type="text" placeholder="Search" className="w-full  px-4 border text-sm border-gray-400 rounded-l-full outline-none" />
                                    ...
                                    searchSuggestion.map((text, idx) => {
                                        return (
                                            <div className="flex items-center px-4 hover:bg-gray-100">
                                                <CiSearch size="24px" />
                                                <li key={idx} className="px-2 py-1 cursor-pointer text-md font-medium">{text}</li>
                                            </div>
                                        )
                                    })
                                }
